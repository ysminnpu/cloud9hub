  ...
  var workspaceName = potentiallyBadPathName[potentiallyBadPathName.length-1];
   ...
   if(typeof req.app.get('runningWorkspaces')[req.user + '/' + workspaceName] === 'undefined'){
       getNextAvailablePort(function(nextFreePort){
            console.log("Starting " + __dirname + '/../../cloud9/bin/cloud9.sh for workspace ' + workspaceName + " on port " + nextFreePort);
       
            var workspace = spawn(__dirname + '/../../cloud9/bin/cloud9.sh', ['-w', __dirname + '/../workspaces/' + req.user + '/' + workspaceName, '-l', '0.0.0.0', '-p', nextFreePort], {detached: true});
